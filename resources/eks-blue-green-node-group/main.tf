...
terraform {  
   backend "s3" {   
      ...
      key            = "eks-blue-green-node-group/terraform.tfstate"               
      region         = "us-east-1"                           
...
locals {
  ...
  deployment_nodegroup      = "blue_green"   # deploy in a specific area. to see the label  do kubectl describe and no number
...
module "eks-control-plane" {
  source       = "../../modules1/eks-blue-green-node-group"
